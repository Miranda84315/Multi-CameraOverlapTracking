...
def write_json(all_results, outputpath, icam, for_eval=False):
    '''
    ...
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            frame = int(im_name.split('/')[-1].split('.')[0].split('_')[-1])
            xmax = 0
            xmin = 1920
            ymax = 0
            ymin = 1080
            for ind in range(0, len(keypoints), 3):
                if keypoints[ind] > xmax:
                    xmax = int(keypoints[ind] + 20)
                if keypoints[ind] < xmin:
                    xmin = int(keypoints[ind] - 20)
            for ind in range(1, len(keypoints), 3):
                if keypoints[ind] > ymax:
                    ymax = int(keypoints[ind] + 20)
                if keypoints[ind] < ymin:
