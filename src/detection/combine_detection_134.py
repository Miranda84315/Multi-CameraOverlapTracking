...
save_img = detection_root + 'imgcam134/'
...
def main():
    ...
    for current_frame in range(startFrame, endFrame):
        detection = np.array([detections[i, [0, 7, 8]] for i in range(len(detections)) if (detections[i, 1] == (current_frame + 1)) and (int(detections[i, 0]) == 1 or int(detections[i, 0]) == 4 or int(detections[i, 0]) == 3)])
        ...
        original_index = np.array([i for i in range(len(detections)) if (detections[i, 1] == (current_frame + 1)) and (int(detections[i, 0]) == 1 or int(detections[i, 0]) == 4 or int(detections[i, 0]) == 3)])
        # delete lonely point
