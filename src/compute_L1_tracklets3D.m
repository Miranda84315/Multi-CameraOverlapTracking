...
% get features from camera1~camera4
    ...
    for iCam = 1:opts.num_cam
        ...
        detections_temp = load(fullfile(opts.dataset_path, 'detections', sprintf('cam%d.mat',iCam)));
    ...
    load(fullfile(opts.dataset_path, 'detections', sprintf('camera_all.mat')));
    ...
    for window_start_frame   = start_frame : opts.tracklets.window_width : end_frame
        ...
        tracklets = createTracklets3D(opts, filteredDetections, filteredFeatures, window_start_frame, window_end_frame, tracklets);
